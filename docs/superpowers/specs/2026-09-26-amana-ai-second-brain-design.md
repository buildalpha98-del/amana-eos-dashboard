...
| `regulator` | Curated list in `src/lib/knowledge/regulator-sources.ts` + monthly cron `knowledge-regulator-refresh` | *Staying Healthy* (NHMRC), ASCIA action plans, Children's Services Award summary (Fair Work), first aid (Safe Work), NSW/VIC regulator pages. Fetch (browser-like User-Agent — several .gov.au sites drop connections for unknown bot UAs), follow at most two redirects and only to allow-listed hosts, then `extractTextFromBuffer`. **ACECQA (NQS, National Regs, MTOP, ratios, quals, notifications) is behind a Cloudflare JS challenge that Node `fetch` cannot pass (verified 2026-09-26)** — those references are NOT in the curated list; the admin loads the ACECQA PDFs (NQS guide, National Regulations guide, MTOP v2.0) through the `manual` upload, which is the same trust boundary. Distinct from the existing `regulatory-monitor` cron (which AI-scans for *changes* and files a report); this adapter indexes the *reference text* |
...
|---|---|---|
| **1 — Content + safety** | Schema + migration (pgvector, `KnowledgeSource`/`Chunk`/`SyncRun`/`AssistantTurn`, `AiUsage.userId` nullable, `Service.content.staffNotes` stripped from the parent payload by `toParentContent()`); pipeline (chunk + embed + tsvector); **hybrid RRF search with `buildKnowledgeScope` enforced in SQL and the tier heuristic + `tierOverride`** (moved here from slice 2 — importing centre/state documents without scope would be a regression); adapters `handbook`, `help_article`, `policy_upload`, `centre_facts`, `lms_module`, `regulator`, `manual`, `backfill`; SharePoint local export + import (§5.1) run against prod; retire the old store per §3.2; role-gate `buildDashboardContext()`; `search_knowledge` over the new store; knowledge console (kind/tier/status, sync, override, exclude, reindex); CI on `pgvector/pgvector:pg16` | Bot is more useful and no longer dangerous |
| **2 — Retrieval policy** | Strict/refuse/general modes, `safety-intent.ts`, relevance thresholds, `resolveEscalation`, citations as chips, `AssistantTurn` logging, admin "test a question" view | Answer policy + auditability |
