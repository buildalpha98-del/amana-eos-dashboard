...
Fix 14 user-reported visible bugs plus 1 auth bug (Report Issue GET), shipping as a single feature branch / single PR. Each fix is root-cause, not a symptom patch. Each UI-observable fix gets a regression test or Playwright scenario in the same PR.
...
| 2 | **Recruitment / New Vacancy** | Modal open → `d.map is not a function` crash | [NewVacancyModal.tsx:30](../../src/components/recruitment/NewVacancyModal.tsx): `return d.services \|\| d;` — if `/api/services` returns `{ data: [...] }` or a paginated wrapper without a `services` key, falls back to the object `d` which has no `.map` | Replace with an array-defensive parse: `const list = Array.isArray(d) ? d : (d.services ?? d.data ?? []);` — and check `/api/services` response shape for ground truth | Component test: mock `/api/services` returning `{ data: [...] }` and verify no crash |
...
| 5 | **Documents** | Open a document → "FORBIDDEN" | Auth check rejects; either role list too strict for the opener, or ownership check miswired, or signed-URL generation fails | Audit the GET route for `/api/documents/[id]`. Check role gate + service-scope gate. Confirm whether the user should have access (based on role/service/role_permissions). Fix the check. | API test: each role (admin, owner, coordinator, member, staff) → correct 200/403 matrix |
...
| 11 | **Postcode field** | Accepts only 3 chars (user report) — should be 4 | Enrol steps already use `slice(0, 4)` ([ParentDetailsStep.tsx:118](../../src/components/enrol/steps/ParentDetailsStep.tsx:118), [ChildDetailsStep.tsx:159](../../src/components/enrol/steps/ChildDetailsStep.tsx:159)). So the bug is in a **different** form — likely CRM lead, services form, or parent account page | Grep all postcode inputs; find the one with `slice(0, 3)`, `maxLength={3}`, or `z.string().max(3)`; change to 4 | Unit test: submit 4-char postcode, assert accepted |
...
**Note on #4, #6, #9, #10, #13:** root cause requires live reproduction. Diagnostic step happens in the implementation phase, not during spec-writing.
...
For bugs with certain root causes (#1, #2, #3, #7, #11, #12, #14) — go straight to fix + test.

For bugs needing live reproduction (#4, #5, #6, #8, #9, #10, #13):
...
1. **Schema / validation fixes first** (#1 Contracts, #3 Timesheets, #11 Postcode) — backend, limited blast radius
2. **Auth fixes** (#5 Documents FORBIDDEN, #14 Report Issue GET)
3. **Hook / query fixes** (#7 Pulse, #8 Communication, #12 LMS tick)
4. **UI wiring fixes** (#2 Recruitment defensive parse, #13 Issues filter)
5. **Live-diagnosis bugs** (#4, #6, #9, #10) — do these last since they require the longest cycle (repro → trace → fix → test)

Each fix = its own commit on the feature branch, so the PR has a clean bisect history.

## Acceptance criteria (sub-project done when)

- [ ] All 14 bugs + Report Issue auth fix shipped on a feature branch
...
- [ ] PR description lists each bug, the root cause, the fix, and the test added
...
- **Q2 (Bug #6 Training module):** does the module detail route `/lms/courses/[id]` / `/training/[id]` exist today? If not, this is a missing route — defer to Sub-project 5 and disable the click in this sub-project, OR escalate to user.
- **Q3 (Bug #11 Postcode):** which specific form has the 3-char limit? Grep suggests it's not in the enrolment flow.
- **Q4 (Bug #13 Issues Show Closed):** is the bug in the API query filter or in the list rendering? Need to read the useIssues hook.
