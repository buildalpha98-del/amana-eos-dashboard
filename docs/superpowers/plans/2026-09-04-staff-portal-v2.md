...
- 2026-09-05: Close-out decisions from Jayden applied — fa36201c (head_office joins Organisation settings incl. the cert matrix), 76b7affe (EH leave joins the roster overlay via a 5-min cached feed, same scoping as internal leave), plus every remaining direct notification writer migrated to notifyUsers() (23 sites, push now fires system-wide). Zero recorded follow-ups outstanding. Suite 6618 green. Prod verified on the final build (workforce route + manifest fingerprints).
