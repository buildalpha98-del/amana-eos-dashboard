...
- [ ] **Step 2: Implement.** In `campaign/send/route.ts`, after the dedupe loop: look up `getSuppressedEmails(recipients.map(r => r.email))` and filter with an explicit lowercase compare — `recipients.filter((r) => !suppressed.has(r.email.toLowerCase()))` (the helper returns a LOWERCASED set; a case-sensitive `has()` silently filters nothing). Keep `suppressedCount`; empty-after-filter → `ApiError.badRequest("All recipients are suppressed or unsubscribed")`; enquiry branch → `ApiError.conflict("This recipient has unsubscribed or bounced — email them individually if genuinely needed")`. Write `externalIdType` in BOTH DeliveryLog.create calls (success + failure paths — on failure `externalIdType` stays undefined since no send happened). Include `suppressedCount` in the success response JSON and inside `payload` (augment the stored `raw` object as `{...raw, _suppressedCount: suppressedCount}` — namespaced so replays aren't confused). Mirror the filter in `recipient-count`.

- [ ] **Step 2b: Stamp the OTHER Brevo send site.** `src/app/api/cowork/email/send/route.ts` (~lines 144/170) also sends via `sendTransactionalEmail` and writes `DeliveryLog.externalId = messageId` — without `externalIdType`, webhook events for cowork-sent mail would never correlate. Add `externalIdType: "brevo_message"` to its DeliveryLog.create data (one line; keeps the webhook's discriminator strict instead of null-matching). Grep for any other `sendTransactionalEmail`/`sendCampaignEmail` call sites that create DeliveryLog rows and stamp them the same way.
...
- [ ] **Step 1: Failing tests:** response now includes `stats.uniqueOpens` and `stats.uniqueClicks`; dailyVolume computed from a full-window query, not the 50-row slice (assert the new `findMany`/`groupBy` is called with the window `where`, and the 50-row `recentSends` query remains separate).
- [ ] **Step 2: Implement:** add ONE `prisma.emailEvent.groupBy({ by: ["type"], where: { createdAt: { gte: since }, type: { in: ["opened", "clicked"] } }, _count: true })`. **Label the metrics UNIQUE, not total**: the webhook's `(messageId, type, email)` dedupe deliberately collapses repeat opens/clicks per recipient (and campaign events share a `camp:<id>` messageId), so the stored counts ARE unique-recipient counts — don't promise totals the data model can't express. Fix dailyVolume with a dedicated `findMany({ where: { createdAt: { gte: since } }, select: { createdAt: true } })` binned in JS (matches current binning code). UI: two new StatCards ("Unique opens (30d)", "Unique clicks (30d)") + a muted caption "Tracking starts from the Phase 3 deploy — earlier sends have no events".
...
- [ ] Push, `gh pr create` — PR body MUST include the deploy checklist: set `BREVO_WEBHOOK_SECRET` in Vercel, configure the webhook URL in Brevo (all event types), then smoke-test: send a campaign, open it, confirm an EmailEvent row exists **with `deliveryLogId` non-null** (the correlation, not just event arrival) + analytics tick up. Also note for Phase 4: the <50-recipient transactional path puts all recipients in one `to:` array (addresses visible to each other) — pre-existing, out of scope here, needs per-recipient sends or BCC batching.
