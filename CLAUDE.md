...
- **UI**: staff profile `RampSection` (only when a ramp exists — the page passes `hasRamp`, the pill hides otherwise; deep links use `#section-ramp`), Staff Lifecycle → "90-day ramp" tab (`RampBoardTab`). Hooks in `src/hooks/useRamp.ts` (`useRamp` treats 404 as "no ramp", not an error).
