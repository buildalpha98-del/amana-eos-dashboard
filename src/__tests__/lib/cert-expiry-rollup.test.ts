...
function cert(
  ...
  return {
    ...
    userId,
    type,
    expiryDate: offsetDays === null ? null : daysFrom(ASOF, offsetDays),
  };
}
...
describe("bucketCertExpiryByService", () => {
  ...
  it("excludes serviceless personal certs (serviceId=null) from the rollup", () => {
    // serviceId is nullable since 2026-06-05 — a personal cert with no
    // service can't be attributed to a centre row.
    const out = bucketCertExpiryByService(
      [cert(null, "u-1", "wwcc", -1)],
      ASOF,
    );
    expect(out.services).toEqual([]);
    expect(out.orgTotals.expired).toBe(0);
  ...
  it("treats null expiryDate as valid-forever (never a bucket)", () => {
    const out = bucketCertExpiryByService(
      [
        cert("svc-1", "u-1", "wwcc", null), // valid forever
        cert("svc-1", "u-2", "wwcc", -1), // expired — only this counts
      ],
      ASOF,
    );
    expect(out.services).toHaveLength(1);
    expect(out.services[0].totals).toEqual({
      expired: 1,
      critical: 0,
      warning: 0,
      upcoming: 0,
