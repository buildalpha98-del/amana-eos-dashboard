...
describe("GET /api/compliance/cert-expiry-rollup", () => {
  ...
  it("filters out superseded and centre-level certs at the SQL boundary", async () => {
    ...
    // Org-wide (no single-service filter), but serviceless personal
    // certs are excluded — they can't be attributed to a centre row.
    expect(findManyCall.where.serviceId).toEqual({ not: null });
  });
