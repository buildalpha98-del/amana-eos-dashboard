...
import { GET, PATCH } from "@/app/api/services/[id]/content/route";
import { _clearUserActiveCache } from "@/lib/server-auth";
import { syncAfterResponse } from "@/lib/knowledge/hooks";
import { SERVICE_CONTENT_DEFAULTS } from "@/lib/service-content-shared";
...
describe("PATCH /api/services/[id]/content — knowledge sync", () => {
  it("fires the centre_facts sync after a successful write", async () => {
    mockSession({ id: "u2", name: "Owner", role: "owner" });
    prismaMock.service.findUnique.mockResolvedValue({ id: SVC_A });
    ...
    const res = await PATCH(
      createRequest("PATCH", `/api/services/${SVC_A}/content`, {
        body: { content: SERVICE_CONTENT_DEFAULTS },
      }),
      { params: Promise.resolve({ id: SVC_A }) },
    );

    expect(res.status).toBe(200);
    expect(syncAfterResponse).toHaveBeenCalledWith("centre_facts", expect.any(Function));
  });
});
