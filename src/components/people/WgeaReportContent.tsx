...
/**
 * WGEA workforce-composition report preview + CSV export.
 * Rendered as the "WGEA Report" tab of /workforce-reports (moved out of
 * the retired /wgea-report page, 2026-07-05).
 *
...
export function WgeaReportContent() {
  const [anonymise, setAnonymise] = useState(true);
  const [includeInactive, setIncludeInactive] = useState(false);

  const { data, isLoading, error } = useQuery<WgeaResponse, ApiResponseError>({
    // Primitive values in the key (not a filter object) so re-renders hit
    // the cache — per the client-hook standards.
