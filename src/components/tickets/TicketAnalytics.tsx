      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Avg First Response</span>
          </div>
          <p className="text-2xl font-bold text-gray-900">{formatHours(data.avgFirstResponseHours)}</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-600" />
            <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Avg Resolution</span>
          </div>
          <p className="text-2xl font-bold text-gray-900">{formatHours(data.avgResolutionHours)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            {Object.entries(data.byPriority).sort(([,a], [,b]) => b - a).map(([priority, count]) => (
              <div key={priority} className="flex items-center gap-3">
                <span className="text-xs font-medium text-gray-600 w-16 capitalize">{priority}</span>
          ...
          <div className="space-y-3">
            {Object.entries(data.byStatus).sort(([,a], [,b]) => b - a).map(([status, count]) => (
              <div key={status} className="flex items-center gap-3">
                <span className="text-xs font-medium text-gray-600 w-24">{statusLabels[status] || status}</span>
                <div className="flex-1 h-6 bg-gray-100 rounded-full overflow-hidden">
                  <div
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          {data.byCentre.length > 0 ? (
            ...
            <p className="text-sm text-gray-400 italic">No centre data</p>
          )}
        </div>
          ...
          {data.agentWorkload.length > 0 ? (
            <div className="space-y-2">
              {data.agentWorkload.map((agent, i) => (
                <div key={agent.id} className="flex items-center gap-3 py-1.5 px-2 rounded-md hover:bg-gray-50">
                  <span className={cn(
                  ...
                  <span className="text-sm font-semibold text-gray-900">{agent.ticketCount} tickets</span>
            ...
            <p className="text-sm text-gray-400 italic">No assigned tickets</p>
          )}
        </div>
      </div>
    </div>
