  ...
  return (
    <div className="bg-card rounded-xl border p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h4 className="font-semibold text-foreground">BSC Growth Tracker</h4>
        ...
        <div className="text-right">
          <div className="text-2xl font-bold text-foreground">
            {data.totalEnrolled}
            <span className="text-sm text-muted font-normal"> / {data.totalTarget}</span>
          </div>
          ...
          <tbody className="divide-y divide-border/50">
            {data.centres.map((c) => (
              <tr key={c.serviceId} className={c.bscEnrolled === 0 ? "bg-red-50/50" : ""}>
                <td className="py-2 font-medium text-foreground">{c.serviceName}</td>
                <td className="py-2 text-right text-foreground/80">{c.bscEnrolled}</td>
                <td className="py-2 text-right text-muted">{c.bscTarget}</td>
