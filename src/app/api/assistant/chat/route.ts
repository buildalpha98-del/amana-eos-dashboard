...
export const POST = withApiAuth(async (req, session) => {
  ...
  const systemPrompt = [
    ...
    "search — e.g. *From QA2 Administration of First Aid Policy →",
    "[section]:*, *From the Parent Handbook → [section]:*, *From the",
    "Amana Way → [section]:*. Never default to 'Amana Way' if the",
    "match came from a different document. Then give a concise,",
    "actionable answer. Quote key procedural lines verbatim where",
    "precision matters (deadlines, escalation paths, policy wording).",
    "",
