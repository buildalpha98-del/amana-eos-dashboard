...
export const POST = withApiAuth(async (req, session) => {
  ...
  const systemPrompt = [
    ...
    "search, and link directly to the document so staff can open the",
    "source PDF/DOCX in one click. Every search result includes an",
    "OpenUrl field — turn the title into a markdown link to that URL.",
    "Format: *From [QA2 Administration of First Aid Policy](/documents/<id>) → [section]:*",
