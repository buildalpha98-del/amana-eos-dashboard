 ...
 * Manager's at-a-glance view of all centres.
 *
...
export const GET = withApiAuth(async (_req, session) => {
  ...
  // the per-service endpoint (PR #69), just without a single-service
  // filter. Serviceless personal certs (serviceId=null, allowed since
  // 2026-06-05) can't be attributed to a centre row, so they're
  // excluded at the SQL boundary. Null expiryDate ("valid forever")
  // never matches the lte filter.
