...
const actionSchema = z.object({
  ...
  /**
   * Optional client-supplied timestamp (ISO 8601). When the client queued
   * the action offline at 3:00pm and the queue drains at 3:42pm, we want
   * the recorded sign-in time to be 3:00pm, not 3:42pm.
   *
   * Server clamps to "now" if the supplied time is in the future, and
   * rejects timestamps older than 24h to prevent replay-shifting attacks.
   */
  occurredAt: z.string().datetime().optional(),
  /**
   * UUID generated by the client when the action was first attempted.
   * Used for activity-log idempotency. The natural unique constraint
   * (childId, serviceId, date, sessionType) already provides exact-once
...
/**
 * Validate + clamp a caller-supplied `occurredAt` to a sensible window.
 * Future timestamps → now. Older than 24h → rejected (caller's queue is
 * stale enough that a manual retake makes more sense than backfilling).
 */
function resolveActionTime(occurredAt?: string): Date {
  const now = new Date();
  ...
  if (ageMs > 24 * 60 * 60 * 1000) {
    throw ApiError.badRequest(
      "occurredAt is more than 24h old — please retake this action.",
    );
  }
  return parsed;
}
