...
export const GET = withApiAuth(async (req, session) => {
  ...
  try {
    ...
    // Get service names for centre counts
    ...
    // Average days in each stage (for completed transitions)
    ...
    for (const e of allEnquiries) {
      const days = (now.getTime() - e.stageChangedAt.getTime()) / (1000 * 60 * 60 * 24);
    ...
    for (const [stage, data] of Object.entries(stageDaysAccum)) {
      avgDaysByStage[stage] = Math.round((data.total / data.count) * 10) / 10;
    }
