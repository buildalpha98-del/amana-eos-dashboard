...
export async function sendEnrolmentReceivedNotification(
  ...
  try {
    ...
    const subject = `New sibling enrolment application — ${app.childFirstName} ${app.childLastName}`;
...
export async function sendEnrolmentApprovedNotification(
  ...
  try {
    ...
    const subject = `Enrolment approved — ${app.childFirstName} ${app.childLastName}`;
    ...
    await sendEmail({ to: app.family.email, subject, html });
  } catch (err) {
...
export async function sendEnrolmentDeclinedNotification(
  ...
  try {
    ...
    const subject = `Enrolment application update — ${app.childFirstName} ${app.childLastName}`;
    const html = parentEmailLayout(`
      <h2 style="margin:0 0 12px;color:#111827;font-size:18px;font-weight:600;">
